...
int main(int argc, const char **argv)
{
    ...
    modulePreProcessForInformation(
        *Mod, top_str, BiOp_Info_name2list_map, IRFunc2BeginLine, FuncParamLine2OutermostSize,
        LoopLabel2DrivenArrayDimensions, IRLoop2LoopLabel, IRLoop2OriginTripCount,
        LoopIRName2NextLevelSubLoopIRNames, LoopIRName2Depth, LoopIRName2Array,
        TargetExtName2ArrayInfo, Loop2PotentialUnrollFactors, debugFlag);
    ...
    for (int i = 0; i < 1000; i++)
    {
        ...
        modulePreProcessForFPGAHLS_optimization(
            *Mod_tmp, top_str, cntStr, BiOp_Info_name2list_map, IRFunc2BeginLine,
            FuncParamLine2OutermostSize, LoopLabel2DrivenArrayDimensions, IRLoop2LoopLabel,
            IRLoop2LoopLabel_eval, IRLoop2OriginTripCount, IRLoop2OriginTripCount_eval,
            LoopIRName2NextLevelSubLoopIRNames, LoopIRName2Depth, LoopIRName2Array,
            TargetExtName2ArrayInfo, Loop2PotentialUnrollFactors, LoopLabel2UnrollFactor,
            debugFlag);
        ...
        while (unrollNotChanged)
        {
            ...
            modulePreProcessForPreEvaluation(
                *Mod_tmp_eval, top_str, cntStr, desginconfig_0, BiOp_Info_name2list_map,
                IRFunc2BeginLine, FuncParamLine2OutermostSize, LoopLabel2DrivenArrayDimensions,
                IRLoop2LoopLabel, IRLoop2LoopLabel_eval, IRLoop2OriginTripCount,
                IRLoop2OriginTripCount_eval, LoopIRName2NextLevelSubLoopIRNames, LoopIRName2Depth,
                LoopIRName2Array, TargetExtName2ArrayInfo, Loop2PotentialUnrollFactors,
                LoopLabel2UnrollFactor, muxWithMoreThan32, muxTooMuchLoopIRName, debugFlag);
            ...
            if (muxWithMoreThan32)
            {
                ...
                int next_factor =
                    findNextUnrollFactor(slow_LoopIRName, LoopLabel2UnrollFactor[opt_LoopLabel],
            ...
            else
            {
                ...
                auto hi_withdirectivetimingresourceevaluation =
                    new HI_WithDirectiveTimingResourceEvaluation(
                        ...
                        desginconfig_0, logName_evaluation.c_str(), "BRAM_info_0",
                        logName_array.c_str(), top_str.c_str(), IRLoop2LoopLabel_eval,
                        IRLoop2OriginTripCount, LoopLabel2II, LoopLabel2UnrollFactor,
                        FuncParamLine2OutermostSize, IRFunc2BeginLine, BiOp_Info_name2list_map,
                        debugFlag);
                ...
                {
                    if (hi_withdirectivetimingresourceevaluation->LoopLabel2Latency.find(
                            it.second) !=
                        hi_withdirectivetimingresourceevaluation->LoopLabel2Latency.end())
                    {
                        checkedConfigs << "    " << it.second << " ====> "
                                       << hi_withdirectivetimingresourceevaluation
                                              ->LoopLabel2Latency[it.second]
                                       << it.second << " ====> "
                                       << hi_withdirectivetimingresourceevaluation
                                              ->LoopLabel2Resource[it.second]
                                       << "\n";
                    }
                }
                     ...
                     hi_withdirectivetimingresourceevaluation->FuncArray2PartitionBenefit)
                {
                    if (!pair.second) // no benefit
                    {
                        ...
                        checkedConfigs << "Array [" << pair.first.second << "] in Func: ["
                                       << pair.first.first
                                       << "] don't need partitioning. Re-run evaluation without "
                                          "partitioning. \n";
                        reRun = 1;
                    }
                }
                ...
                slow_LoopIRName = findSlowestLastLevelLoop_InSlowestTopLoop(
                    IRLoop2LoopLabel, hi_withdirectivetimingresourceevaluation->LoopLabel2Latency,
                ...
                bool directUnroll = hi_withdirectivetimingresourceevaluation
                                            ->LoopLabel2IterationLatency[opt_LoopLabel] -
                                        1 ==
                                    0;

                if (!directUnroll &&
                    hi_withdirectivetimingresourceevaluation->LoopLabel2AchievedII.find(
                        opt_LoopLabel) ==
                        hi_withdirectivetimingresourceevaluation->LoopLabel2AchievedII.end() &&
                    hi_withdirectivetimingresourceevaluation
                            ->LoopLabel2IterationLatency[opt_LoopLabel] >=
                        hi_withdirectivetimingresourceevaluation
                            ->LoopLabel2SmallestII[opt_LoopLabel])
                {
                    llvm::errs() << "for config#" << i + 1
                                 << " suggested start loop pipeline II for it is :"
                                 << hi_withdirectivetimingresourceevaluation
                                        ->LoopLabel2IterationLatency[opt_LoopLabel]
                                 << "\n";
                    desginconfig_0.insertLoopPipeline(
                        opt_LoopLabel, hi_withdirectivetimingresourceevaluation
                                           ->LoopLabel2IterationLatency[opt_LoopLabel]);
                }
                else
                {
                    if (!directUnroll && hi_withdirectivetimingresourceevaluation
                                                 ->LoopLabel2AchievedII[opt_LoopLabel] >
                                             hi_withdirectivetimingresourceevaluation
                                                 ->LoopLabel2SmallestII[opt_LoopLabel])
                    {
                        llvm::errs() << "LoopLabel2AchievedII="
                                     ...
                                     << hi_withdirectivetimingresourceevaluation
                                            ->LoopLabel2SmallestII[opt_LoopLabel]
                                     << "\n";
                        // llvm::errs()  << "for config#" << i+1 << "suggested next loop pipeline II
                        // for it is :" <<
                        // (hi_withdirectivetimingresourceevaluation->LoopLabel2AchievedII[opt_LoopLabel]
                        // +
                        // hi_withdirectivetimingresourceevaluation->LoopLabel2SmallestII[opt_LoopLabel])/2
                        // << "\n";
                        llvm::errs() << "for config#" << i + 1
                                     ...
                                     << (hi_withdirectivetimingresourceevaluation
                                         ...
                                         1)
                                     << "\n";
                        // desginconfig_0.insertLoopPipeline(opt_LoopLabel,
                        // (hi_withdirectivetimingresourceevaluation->LoopLabel2AchievedII[opt_LoopLabel]
                        // +
                        // hi_withdirectivetimingresourceevaluation->LoopLabel2SmallestII[opt_LoopLabel])/2);
                        desginconfig_0.insertLoopPipeline(
                    ...
                    else
                    {
                        ...
                        int next_factor = findNextUnrollFactor(
                            slow_LoopIRName, LoopLabel2UnrollFactor[opt_LoopLabel],
                            Loop2PotentialUnrollFactors);
                        llvm::errs()
                            << "for config#" << i + 1
