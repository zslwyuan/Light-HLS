...
void modulePreProcessForInformation(
    llvm::Module &Mod, std::string top_str,
    ...
    std::map<std::string, std::vector<int>> &Loop2PotentialUnrollFactors, bool debugFlag)
{
    LLVMTargetRef T;
...
void modulePreProcessForFPGAHLS_optimization(
    llvm::Module &Mod, std::string top_str, std::string cntStr,
    std::map<std::string, Info_type_list> &BiOp_Info_name2list_map,
    std::map<std::string, std::vector<int>> &IRFunc2BeginLine,
    std::map<std::string, int> &FuncParamLine2OutermostSize,
    std::map<std::string, std::set<std::pair<std::string, std::pair<std::string, int>>>>
    ...
    std::map<std::string, int> &LoopLabel2UnrollFactor, bool debugFlag)
{
    LLVMTargetRef T;
...
void modulePreProcessForPreEvaluation(
    llvm::Module &Mod, std::string top_str, std::string cntStr, HI_DesignConfigInfo &desginconfig,
    std::map<std::string, Info_type_list> &BiOp_Info_name2list_map,
    std::map<std::string, std::vector<int>> &IRFunc2BeginLine,
    std::map<std::string, int> &FuncParamLine2OutermostSize,
    std::map<std::string, std::set<std::pair<std::string, std::pair<std::string, int>>>>
    ...
    std::map<std::pair<std::string, std::string>, HI_PragmaArrayInfo> &TargetExtName2ArrayInfo,
    std::map<std::string, std::vector<int>> &Loop2PotentialUnrollFactors,
    std::map<std::string, int> &LoopLabel2UnrollFactor, bool &muxWithMoreThan32,
    std::string &muxTooMuchLoopIRName, bool debugFlag)
{
    ...
    auto hi_MuxInsertionArrayPartition = new HI_MuxInsertionArrayPartition(
        // configFile_str.c_str(),
        desginconfig, top_str.c_str(), FuncParamLine2OutermostSize, IRFunc2BeginLine, debugFlag);
