...
int main(int argc, const char **argv)
{
    ...
    auto hi_ArraySensitiveToLoopLevel = new HI_ArraySensitiveToLoopLevel(
        ...
        LoopLabel2DrivenArrayDimensions, BiOp_Info_name2list_map, debugFlag);
    PM_pre_check.add(hi_ArraySensitiveToLoopLevel);
    print_info("Enable HI_ArraySensitiveToLoopLevel Pass");
    PM_pre_check.run(*Mod_pre);
    enumerateDesignConfiguration("arrayConfigs", IRLoop2LoopLabel, IRLoop2OriginTripCount,
                                 LoopIRName2NextLevelSubLoopIRNames, LoopIRName2Depth,
                                 LoopIRName2Array, TargetExtName2ArrayInfo);
    ...
    for (int i = 0; i < 1; i++)
    {
        ...
        {
            llvm::errs() << "Loop: [" << label_function_array_dim.first
                         << "] drives the partitioning of the array dimensions: \n    ";
            for (auto function_array_dim : label_function_array_dim.second)
            {
                llvm::errs() << " "
                             << "{F[" << function_array_dim.first << "], A["
                             << function_array_dim.second.first << "], D["
                             << function_array_dim.second.second << "]},  ";
            }
            llvm::errs() << "\n";
        }
        ...
        auto hi_MuxInsertionArrayPartition = new HI_MuxInsertionArrayPartition(
            // configFile_str.c_str(),
            desginconfig_0, top_str.c_str(), FuncParamLine2OutermostSize, IRFunc2BeginLine,
            debugFlag);
        ...
        else
        {
            ...
            {
                llvm::errs() << "suggested start loop pipeline II for it is :"
            ...
            {
                if (LoopLabel2II[opt_LoopLabel] !=
                    hi_withdirectivetimingresourceevaluation->LoopLabel2SmallestII[opt_LoopLabel])
                {
                    llvm::errs() << "suggested next loop pipeline II for it is :"
                                 << (LoopLabel2II[opt_LoopLabel] +
                                     hi_withdirectivetimingresourceevaluation
                    ...
                    desginconfig_0.insertLoopPipeline(
                        opt_LoopLabel,
                        (LoopLabel2II[opt_LoopLabel] + hi_withdirectivetimingresourceevaluation
                                                           ->LoopLabel2SmallestII[opt_LoopLabel]) /
                            2);
                    continue;
                }
...
// void enumerateDesignConfiguration::generateLoopConfiguration(std::string LoopName, bool
// pragmaSet)
// {
//   // should be BFS!!
//     if (LoopIRName2NextLevelSubLoopIRNames.find(LoopName) !=
