...
int main(int argc, const char **argv)
{
    ...
    enumerateDesignConfiguration configs_bruteForse(
        "configs_brute", IRLoop2LoopLabel, IRLoop2OriginTripCount,
    ...
    for (auto clockForFunction : potential_clockForFunction_vector)
    {
        pushing_relaxtion_log << "changing clock setting from " << printClocks(clockForFunction0)
                              << " to " << printClocks(clockForFunction) << "\n";
        while (1)
        {
            ...
            if (enableTryClock)
            {
                pushing_relaxtion_log << "EVALUATED update clock for function ["
                                      << slowestFunctioName << "] from "
                                      << printClocks(clockForFunction) << "ns"
            ...
            {
                pushing_relaxtion_log << "EVALUATED update clock for function ["
                                      << slowestFunctioName << "] : No NEED currently.\n";
                // pushing_relaxtion_log.flush();
            }
            ...
            if (tryClock && tryDirective)
            {
                ...
                if (tryClock && !tryDirective)
                {
                    ...
                    pushing_relaxtion_log << "update clock for function [" << slowestFunctioName
                                          ...
                                          << " to " << printClocks(new_clockForFunction) << "ns\n";
                    pushing_relaxtion_log << "update HLS directive for function ["
                                          << slowestFunctioName << "] from \n"
                                          << desginconfig_last << " to \n"
            ...
            else if (tryClock && !tryDirective)
            {
                ...
                else
                {
                    ...
                    desginconfig_last.blockPartitionConfigs.clear();
                    desginconfig_last.LoopLabel2UnrollFactor.clear();
                    pushing_relaxtion_log << "Clock Utilization Violation and no Directive "
                                             "solution, choose Clock solution\n";
                    break;
                    // pushing_relaxtion_log.flush();
                }
            }
