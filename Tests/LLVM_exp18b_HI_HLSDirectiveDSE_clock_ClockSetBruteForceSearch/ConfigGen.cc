...
std::string findSlowestLastLevelLoop_InSlowestTopLoop(
    std::string funcScope, std::map<std::string, std::string> &TopLoopIR2FuncName,
    std::map<std::string, std::string> &IRLoop2LoopLabel,
    std::map<std::string, HI_WithDirectiveTimingResourceEvaluation::timingBase> &LoopLabel2Latency,
    std::map<std::string, std::vector<std::string>> &LoopIRName2NextLevelSubLoopIRNames,
    std::map<std::string, int> &LoopIRName2Depth)
{
    ...
    {
        double curLatency = LoopLabel2Latency[IRLoop2LoopLabel[topIRLoop]].latency *
                            LoopLabel2Latency[IRLoop2LoopLabel[topIRLoop]].clock_period;
        if (curLatency > max_latency)
        {
            max_latency = curLatency;
    ...
    std::map<std::string, int> &LoopIRName2Depth)
{
        ...
        LoopIRName2NextLevelSubLoopIRNames.end())
    {
        ...
        for (auto next_level_IRname : LoopIRName2NextLevelSubLoopIRNames[cur_LoopIRName])
        {
            ...
            {
                double curLatency =
                    LoopLabel2Latency[IRLoop2LoopLabel[next_level_IRname]].latency *
                    LoopLabel2Latency[IRLoop2LoopLabel[next_level_IRname]].clock_period;
                if (curLatency > max_latency)
                {
                    max_latency = curLatency;
...
findSubLoopIRs(std::string cur_LoopIRName, std::map<std::string, std::string> &IRLoop2LoopLabel,
               std::map<std::string, std::vector<std::string>> &LoopIRName2NextLevelSubLoopIRNames)
{
    ...
    {
        auto tmp_IRs =
            findSubLoopIRs(next_level_IRname, IRLoop2LoopLabel, LoopIRName2NextLevelSubLoopIRNames);
...
std::string getSlowestModuleInDataflow(
    ...
    std::string topFunction)
{
    ...
    {
        if (demangleFunctionName(it.first) == topFunction &&
...
bool singleClockDesignEvaluation(
    HI_DesignConfigInfo desginconfig_0, llvm::raw_fd_ostream &checkedConfigs, llvm::Module &Mod,
    designEvalResult &design_eval_result, int &output_next_factor,
    ...
    std::map<std::string, Info_type_list> &BiOp_Info_name2list_map, int config_id,
    std::string top_str, bool debugFlag)
{
    std::error_code EC;
    ...
    // {
    //     llvm::errs() << "Loop: [" << label_function_array_dim.first << "] drives the partitioning
    //     of the array dimensions: \n    "; for (auto function_array_dim :
    //     label_function_array_dim.second)
    //     {
    //     llvm::errs() << " " << "{F[" << function_array_dim.first << "], A[" <<
    //     function_array_dim.second.first << "], D[" << function_array_dim.second.second << "]}, ";
    //     }
    //     llvm::errs() << "\n";
    // }
    ...
    auto hi_MuxInsertionArrayPartition = new HI_MuxInsertionArrayPartition(
        ...
        desginconfig_0, top_str.c_str(), FuncParamLine2OutermostSize, IRFunc2BeginLine, debugFlag);
    ...
    if (hi_MuxInsertionArrayPartition->muxWithMoreThan32)
    {
        ...
        int next_factor = findNextUnrollFactor(
            slow_LoopIRName, LoopLabel2UnrollFactor[opt_LoopLabel], Loop2PotentialUnrollFactors);
        ...
        for (auto loop_array_pair : LoopLabel2DrivenArrayDimensions[opt_LoopLabel])
        {
            ...
            // std::map<std::string, std::set<std::pair<std::string, std::pair<std::string, int>> >>
            desginconfig_0.insertArrayCyclicPartition(demangleFunctionName(funcStr), arrayStr, dim,
                                                      next_factor);
        }
        unrollNotChanged = 0;
    ...
    else
    {
        std::string logName_evaluation =
            "HI_WithDirectiveTimingResourceEvaluation_forCheck_" + cntStr;
        ...
        auto hi_withdirectivetimingresourceevaluation =
            new HI_WithDirectiveTimingResourceEvaluation(
                // configFile_str.c_str(),
                desginconfig_0, logName_evaluation.c_str(), "BRAM_info_0", logName_array.c_str(),
                top_str.c_str(), IRLoop2LoopLabel_eval, IRLoop2OriginTripCount, LoopLabel2II,
                LoopLabel2UnrollFactor, FuncParamLine2OutermostSize, IRFunc2BeginLine,
                BiOp_Info_name2list_map, debugFlag);
        ...
        {
            if (hi_withdirectivetimingresourceevaluation->LoopLabel2Latency.find(it.second) !=
                hi_withdirectivetimingresourceevaluation->LoopLabel2Latency.end())
            {
                checkedConfigs
                    << "    " << it.second << " ====> "
            ...
            if (TopLoopIR2FuncName.find(it.first) != TopLoopIR2FuncName.end())
            {
                checkedConfigs << "    " << it.second
                               << " is top-level loop extracted in function: ["
                               << TopLoopIR2FuncName[it.first] << "] with latency="
                               << hi_withdirectivetimingresourceevaluation
                                      ->FuncName2Latency[TopLoopIR2FuncName[it.first]]
                               << " at clock_period="
                               << hi_withdirectivetimingresourceevaluation
                                      ->FuncName2Latency[TopLoopIR2FuncName[it.first]]
                                      .clock_period
                               ...
                               << " with resource="
                               << hi_withdirectivetimingresourceevaluation
                                      ->FuncName2Resource[TopLoopIR2FuncName[it.first]]
                               << "\n";
            }
            checkedConfigs << "\n";
        }

        std::string slowestFuncName = getSlowestModuleInDataflow(
            hi_withdirectivetimingresourceevaluation->FuncName2Latency,
            hi_withdirectivetimingresourceevaluation->FuncName2Resource, top_str);

        checkedConfigs << "    Slowest Function : " << slowestFuncName << "\n";

        checkedConfigs << "    Top Function Latency: "
                       << hi_withdirectivetimingresourceevaluation->top_function_latency
                       << " cycle\n\n";
        checkedConfigs << "    Top Function Delay: "
                       << hi_withdirectivetimingresourceevaluation->top_function_latency *
                              desginconfig_0.clock_period
                       << " ns\n\n";
        checkedConfigs << "    Top Function Resource: "
                       << hi_withdirectivetimingresourceevaluation->topFunction_resource << "\n\n";
        ...
        design_eval_result.FuncName2Latency =
            hi_withdirectivetimingresourceevaluation->FuncName2Latency;
        design_eval_result.FuncName2Resource =
            hi_withdirectivetimingresourceevaluation->FuncName2Resource;
        design_eval_result.LoopLabel2Latency =
            hi_withdirectivetimingresourceevaluation->LoopLabel2Latency;
        design_eval_result.LoopLabel2Resource =
            hi_withdirectivetimingresourceevaluation->LoopLabel2Resource;
        design_eval_result.LoopLabel2IterationLatency =
            hi_withdirectivetimingresourceevaluation->LoopLabel2IterationLatency;
        design_eval_result.LoopLabel2SmallestII =
            hi_withdirectivetimingresourceevaluation->LoopLabel2SmallestII;
        design_eval_result.LoopLabel2AchievedII =
            hi_withdirectivetimingresourceevaluation->LoopLabel2AchievedII;
        design_eval_result.FuncArray2PartitionBenefit =
            hi_withdirectivetimingresourceevaluation->FuncArray2PartitionBenefit;
...
bool multipleClockDesignEvaluation(
    HI_DesignConfigInfo desginconfig_last, llvm::raw_fd_ostream &checkedConfigs, llvm::Module &Mod,

    designEvalResult &entire_design_eval_result, std::string &slowestFunctioName,
    double &max_Latency, double &second_max_Latency, double &clock_forSlowestFunc,
    ...
    int &output_next_factor, std::string &output_next_loopLabel,

    std::map<std::string, std::string> &clockForFunction, std::vector<std::string> &FuncNames,
    std::map<std::string, std::string> &IRLoop2LoopLabel,
    std::map<std::string, std::vector<int>> &IRFunc2BeginLine,
    std::map<std::string, int> &FuncParamLine2OutermostSize,
    std::map<std::string, std::vector<int>> &Loop2PotentialUnrollFactors,
    ...
    int config_id, std::string top_str, bool debugFlag)
{
    ...
    for (auto funcName : FuncNames)
    {
        ...
        bool result_bypass = singleClockDesignEvaluation(
            ...
            IRLoop2LoopLabel, IRFunc2BeginLine, FuncParamLine2OutermostSize,
            Loop2PotentialUnrollFactors, LoopLabel2DrivenArrayDimensions, IRLoop2OriginTripCount,
            TopLoopIR2FuncName, LoopIRName2NextLevelSubLoopIRNames, LoopIRName2Depth,
            LoopIRName2Array, BiOp_Info_name2list_map, 0, top_str, debugFlag);

        if (!design_eval_result.success)
            return false;

        llvm::errs() << "function [" << funcName
                     << "] latency=" << design_eval_result.FuncName2Latency[funcName]
                     << " resource=" << design_eval_result.FuncName2Resource[funcName] << "\n";
        if (design_eval_result.FuncName2Latency[funcName].latency *
                design_eval_result.FuncName2Latency[funcName].clock_period >
            max_Latency)
        {
            max_Latency = design_eval_result.FuncName2Latency[funcName].latency *
                          design_eval_result.FuncName2Latency[funcName].clock_period;
        ...
        entire_design_eval_result.FuncName2Latency[funcName] =
            design_eval_result.FuncName2Latency[funcName];
        entire_design_eval_result.FuncName2Resource[funcName] =
            design_eval_result.FuncName2Resource[funcName];
        for (auto it : TopLoopIR2FuncName)
        {
            if (it.second == funcName)
            {
                for (auto IR_Name :
                     findSubLoopIRs(it.first, IRLoop2LoopLabel, LoopIRName2NextLevelSubLoopIRNames))
                {
                    // if (design_eval_result.LoopLabel2Latency.find(IRLoop2LoopLabel[IR_Name]) ==
                    // design_eval_result.LoopLabel2Latency.end())
                    // {
                    //     llvm::errs() << " IR_Name=" << IR_Name << " IRLoop2LoopLabel[IR_Name]="
                    //     << IRLoop2LoopLabel[IR_Name] << "\n"; for (auto it :
                    //     design_eval_result.LoopLabel2Latency)
                    //         llvm::errs() << "    " << it.first << "----" << it.second << "\n";
                    // }
                    // assert(design_eval_result.LoopLabel2Latency.find(IRLoop2LoopLabel[IR_Name])
                    // != design_eval_result.LoopLabel2Latency.end()); if
                    // (design_eval_result.LoopLabel2Resource.find(IRLoop2LoopLabel[IR_Name]) ==
                    // design_eval_result.LoopLabel2Resource.end())
                    // {
                    //     llvm::errs() << " IR_Name=" << IR_Name << " IRLoop2LoopLabel[IR_Name]="
                    //     << IRLoop2LoopLabel[IR_Name] << "\n"; for (auto it :
                    //     design_eval_result.LoopLabel2Resource)
                    //         llvm::errs() << "    " << it.first << "----" << it.second << "\n";
                    // }
                    // assert(design_eval_result.LoopLabel2Resource.find(IRLoop2LoopLabel[IR_Name])
                    // != design_eval_result.LoopLabel2Resource.end()); if
                    // (design_eval_result.LoopLabel2SmallestII.find(IRLoop2LoopLabel[IR_Name]) ==
                    // design_eval_result.LoopLabel2SmallestII.end())
                    // {
                    //     llvm::errs() << " IR_Name=" << IR_Name << " IRLoop2LoopLabel[IR_Name]="
                    //     << IRLoop2LoopLabel[IR_Name] << "\n"; for (auto it :
                    //     design_eval_result.LoopLabel2SmallestII)
                    //         llvm::errs() << "    " << it.first << "----" << it.second << "\n";
                    // }
                    // assert(design_eval_result.LoopLabel2SmallestII.find(IRLoop2LoopLabel[IR_Name])
                    // != design_eval_result.LoopLabel2SmallestII.end()); if
                    // (design_eval_result.LoopLabel2IterationLatency.find(IRLoop2LoopLabel[IR_Name])
                    // == design_eval_result.LoopLabel2IterationLatency.end())
                    // {
                    //     llvm::errs() << " IR_Name=" << IR_Name << " IRLoop2LoopLabel[IR_Name]="
                    //     << IRLoop2LoopLabel[IR_Name] << "\n"; for (auto it :
                    //     design_eval_result.LoopLabel2IterationLatency)
                    //         llvm::errs() << "    " << it.first << "----" << it.second << "\n";
                    // }
                    // assert(design_eval_result.LoopLabel2IterationLatency.find(IRLoop2LoopLabel[IR_Name])
                    // != design_eval_result.LoopLabel2IterationLatency.end());
                    if (design_eval_result.LoopLabel2Resource.find(IRLoop2LoopLabel[IR_Name]) !=
                        design_eval_result.LoopLabel2Resource.end())
                    {
                        entire_design_eval_result.LoopLabel2Latency[IRLoop2LoopLabel[IR_Name]] =
                            design_eval_result.LoopLabel2Latency[IRLoop2LoopLabel[IR_Name]];
                        entire_design_eval_result.LoopLabel2Resource[IRLoop2LoopLabel[IR_Name]] =
                            design_eval_result.LoopLabel2Resource[IRLoop2LoopLabel[IR_Name]];
                    }
                    if (design_eval_result.LoopLabel2SmallestII.find(IRLoop2LoopLabel[IR_Name]) !=
                        design_eval_result.LoopLabel2SmallestII.end())
                        entire_design_eval_result.LoopLabel2SmallestII[IRLoop2LoopLabel[IR_Name]] =
                            design_eval_result.LoopLabel2SmallestII[IRLoop2LoopLabel[IR_Name]];
                    if (design_eval_result.LoopLabel2IterationLatency.find(
                            IRLoop2LoopLabel[IR_Name]) !=
                        design_eval_result.LoopLabel2IterationLatency.end())
                        entire_design_eval_result
                            .LoopLabel2IterationLatency[IRLoop2LoopLabel[IR_Name]] =
                            design_eval_result
                                .LoopLabel2IterationLatency[IRLoop2LoopLabel[IR_Name]];
                    if ((design_eval_result.LoopLabel2AchievedII.find(IRLoop2LoopLabel[IR_Name]) !=
                         design_eval_result.LoopLabel2AchievedII.end()))
                        entire_design_eval_result.LoopLabel2AchievedII[IRLoop2LoopLabel[IR_Name]] =
                            design_eval_result.LoopLabel2AchievedII[IRLoop2LoopLabel[IR_Name]];
                }
            }
        }
        entire_design_eval_result.FuncArray2PartitionBenefit =
            design_eval_result.FuncArray2PartitionBenefit;
        ...
        {
            if (demangleFunctionName(func_resource_pair.first) == top_str ||
                func_resource_pair.first.find(".") == std::string::npos)
            {
    ...
    for (auto funcName : FuncNames)
    {
        if (funcName == slowestFunctioName)
            continue;
        if (MultiClk_FuncName2Latency[funcName].latency *
                MultiClk_FuncName2Latency[funcName].clock_period >
            second_max_Latency)
        {
            second_max_Latency = MultiClk_FuncName2Latency[funcName].latency *
                                 MultiClk_FuncName2Latency[funcName].clock_period;
        }
    }
...
//                       &newclk_total_resource, std::map<std::string,
//                       HI_WithDirectiveTimingResourceEvaluation::timingBase>
//                       &MultiClk_FuncName2Latency, std::map<std::string,
//                       HI_WithDirectiveTimingResourceEvaluation::resourceBase>
//                       &MultiClk_FuncName2Resource, std::string &optClock, HI_DesignConfigInfo &
...
//                       std::pair<std::string, int>> >> &LoopLabel2DrivenArrayDimensions,
//                       std::map<std::string, int> &IRLoop2OriginTripCount,
//                       std::map<std::string, std::string> &TopLoopIR2FuncName,
//                       std::map<std::string, std::vector<std::string>>
//                       &LoopIRName2NextLevelSubLoopIRNames, std::map<std::string, int>
//                       &LoopIRName2Depth, std::map<std::string, std::vector<
//                       std::pair<std::string, std::string>>> &LoopIRName2Array,
//                       std::map<std::string,Info_type_list> &BiOp_Info_name2list_map,
...
//                         );
//         checkedConfigs.flush();
//         llvm::errs() << "function [" << slowestFunctioName << "] latency=" <<
//         design_eval_result.FuncName2Latency[slowestFunctioName] << " resource=" <<
//         design_eval_result.FuncName2Resource[slowestFunctioName] << "\n"; if
//         (design_eval_result.FuncName2Latency[slowestFunctioName].latency *
//         design_eval_result.FuncName2Latency[slowestFunctioName].clock_period <=
//         second_max_Latency + 0.001)
//         {
...
//             }
//             accumulateResource(newclk_func_resource,
//             design_eval_result.FuncName2Resource[slowestFunctioName] ); llvm::errs() << "update
//             clock for function [" << slowestFunctioName << "] to " <<  clockStrs_DSE[curClock_id]
//             << " can resolve the bottleneck\n"; break;
//         }
...
//     if (curClock_id<0)
//     {
//         llvm::errs() << "tuning the clock frequency does not help function [" <<
//         slowestFunctioName << "]\n"; accumulateResource(newclk_total_resource,
//         ori_total_resource); return false;
//     }
//     else
//     {
...
bool tryUpdateSlowestFuncClock(
    ...
    std::string slowestFunctioName, double ori_max_Latency, double ori_second_max_Latency,
    double clock_forSlowestFunc,
    HI_WithDirectiveTimingResourceEvaluation::resourceBase ori_total_resource,
    ...
    std::map<std::string, HI_WithDirectiveTimingResourceEvaluation::resourceBase>
        &MultiClk_FuncName2Resource,
    std::string &optClock, HI_DesignConfigInfo &desginconfig_optClock,
    ...
    int config_id, std::string top_str, bool debugFlag)
{
    ...
    while (curClock_id >= 0)
    {
        ...
        bool tryDirective = tryUpdateHLSDirectives(
            desginconfig_optClock, checkedConfigs, Mod,

            slowestFunctioName, ori_max_Latency, ori_second_max_Latency, ori_total_resource,
            newclk_total_resource, desginconfig_optClock, new_max_Latency,
        ...
        {
            optClock = clockStrs_DSE[curClock_id];
            llvm::errs() << "CAN update clock for function [" << slowestFunctioName << "] to "
                         << clockStrs_DSE[curClock_id] << " can resolve the bottleneck\n";
            break;
        }
...
bool tryUpdateHLSDirectives(
    ...
    std::string ori_slowestFunctioName, double ori_max_Latency, double ori_second_max_Latency,
    HI_WithDirectiveTimingResourceEvaluation::resourceBase ori_total_resource,
    HI_WithDirectiveTimingResourceEvaluation::resourceBase &newloop_total_resource,
    HI_DesignConfigInfo &desginconfig_optLoop, double &new_max_Latency,
    ...
    int config_id, std::string top_str, bool debugFlag)
{
    ...
    while (1)
    {
        ...
        bool eval_success = multipleClockDesignEvaluation(
            desginconfig_last, checkedConfigs, Mod, entire_design_eval_result,

            slowestFunctioName, max_Latency, second_max_Latency, clock_forSlowestFunc,
            newloop_total_resource, MultiClk_FuncName2Latency, MultiClk_FuncName2Resource,
        ...
        checkedConfigs << "utilizationIndex(newloop_total_resource)="
        ...
        {
            checkedConfigs << "cur_Latency=" << max_Latency
                           << "   ori_max_Latency=" << ori_max_Latency << "\n";
        ...
        for (auto pair : entire_design_eval_result.FuncArray2PartitionBenefit)
        {
            if (!pair.second) // no benefit
            {
                ...
                checkedConfigs
                    << "Array [" << pair.first.second << "] in Func: [" << pair.first.first
                    << "] don't need partitioning. Re-run evaluation without partitioning. \n";
                reRun = 1;
        ...
        if (eval_success)
        {
            // if (slowestFunctioName != ori_slowestFunctioName && max_Latency == ori_max_Latency ||
            // max_Latency < ori_max_Latency+0.01) if (max_Latency < ori_max_Latency+0.01) if
            // (slowestFunctioName != ori_slowestFunctioName && max_Latency <= ori_max_Latency+0.01
            // || slowestFunctioName == ori_slowestFunctioName && max_Latency < ori_max_Latency)
            if (HLSDirective_Updated)
            {
                new_max_Latency = max_Latency;
                llvm::errs()
                    << "CAN function [" << ori_slowestFunctioName
                    << " has been optimized to not bottle neck and new bottleneck funtion is ["
                    << slowestFunctioName << "]\n";
            ...
            slow_LoopIRName = findSlowestLastLevelLoop_InSlowestTopLoop(
                slowestFunctioName, TopLoopIR2FuncName, IRLoop2LoopLabel,
                entire_design_eval_result.LoopLabel2Latency, LoopIRName2NextLevelSubLoopIRNames,
                LoopIRName2Depth);
        ...
        bool directUnroll =
            entire_design_eval_result.LoopLabel2IterationLatency[opt_LoopLabel] - 1 == 0 ||
            !eval_success;
        if (!directUnroll && entire_design_eval_result.LoopLabel2AchievedII.find(opt_LoopLabel) ==
                                 entire_design_eval_result.LoopLabel2AchievedII.end())
        {
            assert(entire_design_eval_result.LoopLabel2SmallestII.find(opt_LoopLabel) !=
                   entire_design_eval_result.LoopLabel2SmallestII.end());
        ...
        if (!directUnroll && entire_design_eval_result.LoopLabel2AchievedII.find(opt_LoopLabel) ==
                                 entire_design_eval_result.LoopLabel2AchievedII.end())
        {
            llvm::errs() << "for config#" << config_id + 1
                         << " suggested start loop pipeline II for it is :"
                         << entire_design_eval_result.LoopLabel2IterationLatency[opt_LoopLabel] - 1
                         << "\n";
            ...
            assert(entire_design_eval_result.LoopLabel2SmallestII[opt_LoopLabel] <=
                   entire_design_eval_result.LoopLabel2IterationLatency[opt_LoopLabel]);
            desginconfig_last.insertLoopPipeline(
                opt_LoopLabel, entire_design_eval_result.LoopLabel2IterationLatency[opt_LoopLabel]);
            HLSDirective_Updated = true;
        }
        else
        {
            if (!directUnroll && entire_design_eval_result.LoopLabel2AchievedII[opt_LoopLabel] !=
                                     entire_design_eval_result.LoopLabel2SmallestII[opt_LoopLabel])
            {
                llvm::errs() << "LoopLabel2AchievedII="
                             << entire_design_eval_result.LoopLabel2AchievedII[opt_LoopLabel]
                             << " LoopLabel2SmallestII="
                             << entire_design_eval_result.LoopLabel2SmallestII[opt_LoopLabel]
                             << "\n";
                // llvm::errs()  << "for config#" << config_id+1 << "suggested next loop pipeline II
                // for it is :" << (entire_design_eval_result.LoopLabel2AchievedII[opt_LoopLabel] +
                // entire_design_eval_result.LoopLabel2SmallestII[opt_LoopLabel])/2 << "\n";
                llvm::errs() << "for config#" << config_id + 1
                             << "suggested next loop pipeline II for it is :"
                             << (entire_design_eval_result.LoopLabel2AchievedII[opt_LoopLabel] - 1)
                             << "\n";
                // desginconfig_last.insertLoopPipeline(opt_LoopLabel,
                // (entire_design_eval_result.LoopLabel2AchievedII[opt_LoopLabel] +
                // entire_design_eval_result.LoopLabel2SmallestII[opt_LoopLabel])/2);

                int dis_achieve_smallest =
                    entire_design_eval_result.LoopLabel2AchievedII[opt_LoopLabel] -
                    entire_design_eval_result.LoopLabel2SmallestII[opt_LoopLabel];
            ...
            else
            {
                ...
                int next_factor;
                if (eval_success)
                    next_factor = findNextUnrollFactor(
                        slow_LoopIRName, desginconfig_last.LoopLabel2UnrollFactor[opt_LoopLabel],
                        Loop2PotentialUnrollFactors);
                else
                ...
                llvm::errs() << "for config#" << config_id + 1
                             << "suggested next loop unroll factor for it is :" << next_factor
                             << "\n";
                ...
                for (auto loop_array_pair : LoopLabel2DrivenArrayDimensions[opt_LoopLabel])
                {
                    ...
                    // int>> >>
                    desginconfig_last.increaseArrayCyclicPartition(demangleFunctionName(funcStr),
...
{
    std::string output = "";
    output += "HI_PragmaArrayInfo for: << " + tb.targetName +
              ">> num_dims=" + std::to_string(tb.num_dims) + ", ";
    for (int i = 0; i < tb.num_dims; i++)
    {
        if (tb.cyclic)
            output += "dim-" + std::to_string(i) + "C-s" + std::to_string(tb.dim_size[i]) + "-p" +
                      std::to_string(tb.partition_size[i]) + ",  ";
        else
            output += "dim-" + std::to_string(i) + "B-s" + std::to_string(tb.dim_size[i]) + "-p" +
                      std::to_string(tb.partition_size[i]) + ",  ";
    }
...
enumerateDesignConfiguration::enumerateDesignConfiguration(
    const char *genFile_name, std::map<std::string, std::string> &IRLoop2LoopLabel,
    std::map<std::string, int> &IRLoop2OriginTripCount,
    std::map<std::string, std::vector<std::string>> &LoopIRName2NextLevelSubLoopIRNames,
    std::map<std::string, int> &LoopIRName2Depth,
    std::map<std::string, std::vector<std::pair<std::string, std::string>>> &LoopIRName2Array,
    std::map<std::pair<std::string, std::string>, HI_PragmaArrayInfo> &TargetExtName2ArrayInfo)
    : IRLoop2LoopLabel(IRLoop2LoopLabel), IRLoop2OriginTripCount(IRLoop2OriginTripCount),
    ...
    for (auto loopIRName_dep_pair : LoopIRName2Depth)
    {
        if (loopIRName_dep_pair.second == 1)
        {
            *genFile << "find top-level-loop: IRname:" << loopIRName_dep_pair.first
            ...
            {
                *genFile << "    possible array configuration for: "
                         << printArray(TargetExtName2ArrayInfo[func_array_pair]) << "\n";
                std::string funcStr = func_array_pair.first;
                std::string arrayStr = func_array_pair.second;
                arrayPragmaGenerator array_configurationGen(
                    TargetExtName2ArrayInfo[func_array_pair]);
...
int getClockId(double clock_period, std::string clockStrs[100])
{
    ...
    {
        if (std::stod(clockStrs[i]) - 0.001 < clock_period &&
            std::stod(clockStrs[i]) + 0.001 > clock_period)
        {
            return i;
        }
    }
...
void generateClockFunctionMaps(std::vector<int> &clockIds, std::vector<std::string> &funcNames,
                               std::vector<std::map<std::string, std::string>> &clockFunctionMaps)
{
...
raw_ostream &operator<<(raw_ostream &stream, designEvalResult &tb)
{
    ...
    for (auto it : tb.FuncName2Latency)
    {
        ...
        stream << "        func:" << funcName << " latency=" << it.second
    ...
    int config_id, std::string top_str, bool debugFlag)
{
    ...
    if (!findNextPossibleClockCombination(slowestFunctioName, clockForFunction,
                                          MultiClk_FuncName2Latency, checkedConfigs))
    {
        llvm::errs() << "failed to tune the clock frequency to help function ["
                     << slowestFunctioName << "]\n";
        return false;
    }
    ...
    bool eval_success = multipleClockDesignEvaluation(
        ...
        slowestFunctioName, new_max_Latency, second_max_Latency, clock_forSlowestFunc,
        newclk_total_resource, MultiClk_FuncName2Latency, MultiClk_FuncName2Resource,
        output_next_factor, output_next_loopLabel,

        clockForFunction, FuncNames, IRLoop2LoopLabel, IRFunc2BeginLine,
        FuncParamLine2OutermostSize, Loop2PotentialUnrollFactors, LoopLabel2DrivenArrayDimensions,
        IRLoop2OriginTripCount, TopLoopIR2FuncName, LoopIRName2NextLevelSubLoopIRNames,
        LoopIRName2Depth, LoopIRName2Array, BiOp_Info_name2list_map, 0, top_str, debugFlag);
    checkedConfigs.flush();
...
bool findNextPossibleClockCombination(
    std::string slowestFunctioName, std::map<std::string, std::string> &new_clockForFunction,
    std::map<std::string, HI_WithDirectiveTimingResourceEvaluation::timingBase>
        &MultiClk_FuncName2Latency,
    llvm::raw_fd_ostream &checkedConfigs)
{
    ...
    while (FuncNameForClockUpdate.size() > 0)
    {
        ...
        {
            if (MultiClk_FuncName2Latency[funcName].clock_period *
                    MultiClk_FuncName2Latency[funcName].latency >
                max_latency_in_set)
            {
                max_latency_in_set = MultiClk_FuncName2Latency[funcName].clock_period *
                                     MultiClk_FuncName2Latency[funcName].latency;
                funcName_withMaxLat = funcName;
            }
        }
        int ori_curClock_id =
            getClockId(MultiClk_FuncName2Latency[funcName_withMaxLat].clock_period, clockStrs_DSE);
          ...
          // with the same clock with the original slowest function
            ...
            while (tmp_curClock_id >= 0)
            {
                if (clockSet.find(clockStrs_DSE[tmp_curClock_id]) != clockSet.end())
                {
                    ...
                    checkedConfigs
                        << "     Successfully find the combination of clocks. Clocks are: ";
            ...
            // (2) lower the clocks of those functions with the same clock with the original slowest
            // function
            std::vector<std::string> funcNames;
            for (auto func_clkstr_pair : tmp_clockForFunction)
            {
