...
#define utilizationViolation(A)                                                                    \
    (A.DSP / DSP_limit > 1 || A.FF / FF_limit > 1 || A.LUT / LUT_limit > 1 ||                      \
     A.BRAM / BRAM_limit > 1)
...
#define stringUtilizationIndex(A)                                                                  \
    pushing_relaxtion_log << "DSP:" << A.DSP / DSP_limit << "  FF:" << A.FF / FF_limit             \
                          << "  LUT:" << A.LUT / LUT_limit << "  BRAM:" << A.BRAM / BRAM_limit     \
                          << " current TotalAreaRate:"                                             \
                          << A.DSP / DSP_limit + A.FF / FF_limit + A.LUT / LUT_limit +             \
                                 A.BRAM / BRAM_limit                                               \
                          << "\n";

#define stringUtilization(A)                                                                       \
    pushing_relaxtion_log << "DSP:" << A.DSP << "  FF:" << A.FF << "  LUT:" << A.LUT               \
                          << "  BRAM:" << A.BRAM << "\n";

#define stringUtilizationIndexTo(A, out)                                                           \
    out << "DSP:" << A.DSP / DSP_limit << "  FF:" << A.FF / FF_limit                               \
        << "  LUT:" << A.LUT / LUT_limit << "  BRAM:" << A.BRAM / BRAM_limit << "\n";
...
bool tryUpdateSlowestFuncClock(
    ...
    std::string slowestFunctioName, double max_Latency, double second_max_Latency,
...
class designEvalResult
{
  public:
    ...
    // designEvalResult(
    //     std::map<std::string, HI_WithDirectiveTimingResourceEvaluation::timingBase>
    //     LoopLabel2Latency, std::map<std::string,
    //     HI_WithDirectiveTimingResourceEvaluation::timingBase> FuncName2Latency,
    //     std::map<std::string, HI_WithDirectiveTimingResourceEvaluation::resourceBase>
    //     FuncName2Resource, std::map<std::string,
    //     HI_WithDirectiveTimingResourceEvaluation::resourceBase> LoopLabel2Resource,
    ...
    // ) : LoopLabel2Latency(LoopLabel2Latency), FuncName2Latency(FuncName2Latency),
    // FuncName2Resource(FuncName2Resource),
    //     LoopLabel2Resource(LoopLabel2Resource), LoopLabel2SmallestII(LoopLabel2SmallestII),
    //     LoopLabel2IterationLatency(LoopLabel2IterationLatency),
...
//     {
//         findTheSlowestLoop(irLoopLabel, LoopLabel2Latency, LoopIRName2NextLevelSubLoopIRNames,
//         LoopIRName2Depth);
//     }
    ...
    class loopSetting
    {
      public:
        loopSetting(std::string loopName, bool hasPragma, bool isUnroll, int Val)
            : loopName(loopName), hasPragma(hasPragma), isUnroll(isUnroll), Val(Val)
        {
        }
    ...
    class arraySetting
    {
      public:
        arraySetting(std::string arrayName, bool hasPragma, bool isCyclic, int Val)
            : arrayName(arrayName), hasPragma(hasPragma), isCyclic(isCyclic), Val(Val)
        {
        }
